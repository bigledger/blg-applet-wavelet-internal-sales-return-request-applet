...
import { ApiResponseModel, bl_fi_generic_doc_hdr_RowClass, bl_fi_generic_doc_line_RowClass, bl_fi_generic_doc_link_RowClass, InternalSalesOrderService, Pagination, SubQueryService } from 'blg-akaun-ts-lib';
...
@Component({
  selector: 'app-ko-for-sales-order-item',
  templateUrl: './ko-for-sales-order-item.component.html',
  styleUrls: ['./ko-for-sales-order-item.component.css']
})
export class KoForSalesOrderItemComponent implements OnInit, OnDestroy {
              ...
              newArrData = [...res.data].map((doc) => {
                ...
                doc.bl_fi_generic_doc_links?.forEach(link => {
                  if(
                    ...
                    link.server_doc_type_doc_1_line === "INTERNAL_SALES_ORDER" &&
