...
@Component({
  selector: 'app-internal-sales-credit-note-select-shipping-address',
  templateUrl: './select-shipping-address.component.html',
  styleUrls: ['./select-shipping-address.component.css'],
...
export class SelectShippingAddressComponent extends ViewColumnComponent {
      ...
      getRows: grid => {
        this.subs.sink = this.customerService.getByGuid(
          this.customerGuid, AppConfig.apiVisa).subscribe(a => {
            grid.success({
              rowData: a.data.bl_fi_mst_entity_hdr.addresses_json.shipping_address,
              rowCount: a.data.bl_fi_mst_entity_hdr.addresses_json.shipping_address.length,
            });
          }, err => {
            grid.fail();
          });
  ...
  onRowClicked(shipping_address) {
    this.store.dispatch(InternalSalesReturnActions.selectShippingAddress({ shipping_address }));
  }
