...
@Component({
  selector: "app-knock-off-jobsheet",
  templateUrl: "./knock-off-jobsheet.component.html",
  styleUrls: ["./knock-off-jobsheet.component.css"],
})
export class KnockOffJobsheetComponent extends ViewColumnComponent {
  ...
  retrieveData() {
    ...
    this.subs.sink = this.internalJobsheetService
  ...
  onKO() {
    ...
    this.viewModelStore.dispatch(
      Column2ViewModelActions.setKOForImport_JSDocNo({
        koForJSDocNo: hdrServerDoc,
      })
    );
  ...
  onSelectionChanged(event: SelectionChangedEvent) {
    ...
    this.subs.sink = this.viewModelStore
      .select(Column2ViewSelectors.selectKOForImport_JSDocNo)
      .subscribe((result) => {
        if (result === serverDoc) {
  ...
  onSearch(e: SearchQueryModel) {
    if (!e.isEmpty) {
      const sql = {
        subquery:
          e.queryString +
          `AND queue.server_doc_type_1 = 'INTERNAL_JOBSHEET' AND queue.server_doc_type_2 = 'INTERNAL_SALES_RETURN_REQUEST'`,
        table: e.table,
      };
