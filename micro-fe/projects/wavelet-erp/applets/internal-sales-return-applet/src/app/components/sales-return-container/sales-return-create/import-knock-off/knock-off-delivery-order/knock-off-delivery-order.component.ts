...
import {
  ...
  bl_fi_generic_doc_link_RowClass, InternalOutboundDeliveryOrderService, Pagination,
  SubQueryService
...
@Component({
  selector: 'app-knock-off-delivery-order',
  templateUrl: './knock-off-delivery-order.component.html',
  styleUrls: ['./knock-off-delivery-order.component.css']
})
export class KnockOffDeliveryOrderComponent extends ViewColumnComponent {
  ...
  retrieveData() {
    ...
    this.pagination.conditionalCriteria = [
      { columnName: "calcTotalRecords", operator: "=", value: "true" },
      {
        ...
        operator: "=",
        value: "INTERNAL_OUTBOUND_DELIVERY_ORDER",
      },
      {
    ...
    this.subs.sink = this.internalOutboundDeliveryOrderService
  ...
  onKO() {
    ...
    this.viewModelStore.dispatch(
      Column2ViewModelActions.setKOForImport_DODocNo({
        koForDODocNo: hdrServerDoc,
      })
    );
  ...
  onSelectionChanged(event: SelectionChangedEvent) {
    ...
    this.subs.sink = this.viewModelStore
      .select(Column2ViewSelectors.selectKOForImport_DODocNo)
      .subscribe((result) => {
        if (result === serverDoc) {
  ...
  createNewLineAndLink(
    ...
    this.viewColFacade.showSuccessToast(
      "Jobsheet Successfully Knocked Off"
    );
  }
