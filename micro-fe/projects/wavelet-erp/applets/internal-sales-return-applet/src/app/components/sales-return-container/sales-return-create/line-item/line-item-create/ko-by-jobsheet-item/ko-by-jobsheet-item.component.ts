...
import { ApiResponseModel, bl_fi_generic_doc_hdr_RowClass, InternalJobsheetService, Pagination, SubQueryService } from 'blg-akaun-ts-lib';
...
@Component({
  selector: 'app-ko-by-jobsheet-item',
  templateUrl: './ko-by-jobsheet-item.component.html',
  styleUrls: ['./ko-by-jobsheet-item.component.css']
})
export class KoByJobsheetItemComponent implements OnInit, OnDestroy {
          ...
          apiVisa).subscribe(
            (res) => {
              ...
              newArrData = [...res.data].map((doc) => {
                ...
                doc.bl_fi_generic_doc_links?.forEach(link => {
                  if(
                    // link.server_doc_type_doc_1_line === "INTERNAL_SALES_RETURN" &&
                    link.server_doc_type_doc_2_line === "INTERNAL_JOBSHEET" &&
