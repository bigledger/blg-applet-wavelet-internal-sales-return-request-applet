...
import { ApiResponseModel, bl_fi_generic_doc_hdr_RowClass, bl_fi_generic_doc_line_RowClass, bl_fi_generic_doc_link_RowClass, InternalOutboundDeliveryOrderService, Pagination, SubQueryService } from 'blg-akaun-ts-lib';
...
import { AppletConstants } from 'projects/wavelet-erp/applets/internal-sales-credit-note-applet/src/app/models/constants/applet-constants';
import { LinkActions, PNSActions } from 'projects/wavelet-erp/applets/internal-sales-credit-note-applet/src/app/state-controllers/draft-controller/store/actions';
...
@Component({
  selector: 'app-ko-for-delivery-order-item',
  templateUrl: './ko-for-delivery-order-item.component.html',
  styleUrls: ['./ko-for-delivery-order-item.component.css']
})
export class KoForDeliveryOrderItemComponent implements OnInit, OnDestroy {
  ...
  columnsDefs = [
    { headerName: 'Delivery Order No.', field: 'doc_number', cellStyle: () => ({ 'text-align': 'left' }), maxWidth: 100, checkboxSelection: true },
  ...
  setGridData(criteria?: any) {
    ...
    const datasource = {
      getRows: grid => {
        ...
        this.subs.sink = this.doService.getGenericDocumentHdrLineByCriteria(
          pagination,
          apiVisa).subscribe(
            (res) => {
              ...
              newArrData = [...res.data].map((doc) => {
                ...
                doc.bl_fi_generic_doc_links?.forEach(link => {
                  if(
                    // link.server_doc_type_doc_2_line === "INTERNAL_SALES_CREDIT_NOTE" &&
                    link.server_doc_type_doc_1_line === "INTERNAL_OUTBOUND_DELIVERY_ORDER" &&
