...
import {
  ...
  MyEInvoiceGenDocToIRBSubmissionHistoryService
...
@Component({
  selector: "app-submitted-to-irb",
  templateUrl: "./submitted-to-irb.component.html",
  styleUrls: ["./submitted-to-irb.component.scss"],
  changeDetection: ChangeDetectionStrategy.OnPush,
  providers: [ComponentStore],
})
export class SubmittedToIRBComponent extends ViewColumnComponent {
  protected subs = new SubSink();

  protected compName = "Submitted To IRB";
  ...
  columnsDefs = [
    {
      ...
      field: "bl_fi_my_einvoice_gen_doc_to_irb_submission_history.submission_status",
    ...
    },
    {
      ...
      field: "bl_fi_my_einvoice_gen_doc_to_irb_submission_history.submission_request_date",
      suppressSizeToFit: false,
...

    {
      ...
      field: "bl_fi_my_einvoice_gen_doc_to_irb_submission_history.validation_error",
      comparator: (valueA, valueB) =>
        valueA.toLowerCase().localeCompare(valueB.toLowerCase()),
      suppressSizeToFit: false,
      cellStyle: () => ({ "text-align": "right" }),
...

    {
      ...
      field: "bl_fi_my_einvoice_gen_doc_to_irb_submission_history.request_url",
      comparator: (valueA, valueB) =>
        valueA.toLowerCase().localeCompare(valueB.toLowerCase()),
      suppressSizeToFit: false,
      cellStyle: () => ({ "text-align": "left" }),
...

    {
      ...
      field: "bl_fi_my_einvoice_gen_doc_to_irb_submission_history.request_body",
      comparator: (valueA, valueB) =>
        valueA.toLowerCase().localeCompare(valueB.toLowerCase()),
      suppressSizeToFit: false,
      cellStyle: () => ({ "text-align": "left" }),
...

    {
      ...
      field: "bl_fi_my_einvoice_gen_doc_to_irb_submission_history.request_response",
      comparator: (valueA, valueB) =>
        valueA.toLowerCase().localeCompare(valueB.toLowerCase()),
      suppressSizeToFit: false,
      cellStyle: () => ({ "text-align": "left" }),
  ...
  setGridData() {
    ...
    const datasource = {
      getRows: (grid) => {
        ...
        this.subs.sink = this.myEInvoiceGenDocToIRBSubmissionHistoryService
