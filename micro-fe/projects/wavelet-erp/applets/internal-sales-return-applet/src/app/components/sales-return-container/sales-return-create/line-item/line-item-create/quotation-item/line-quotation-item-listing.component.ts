...
import { Pagination, InternalSalesQuotationService, SubQueryService, ApiResponseModel } from 'blg-akaun-ts-lib';
...
@Component({
  selector: 'app-line-quotation-item-listing',
  templateUrl: './line-quotation-item-listing.component.html',
  styleUrls: ['./line-quotation-item-listing.component.scss']
})
export class LineQuotationItemListingComponent implements OnInit, OnDestroy {
  ...
  columnsDefs = [
    { headerName: 'Sales Quotation No.', field: 'doc_number', cellStyle: () => ({ 'text-align': 'left' }), floatingFilter: true },
  ...
  //       this.subs.sink = this.isqService.getLinesByCriteria(this.pagination, apiVisa).pipe(
  //         mergeMap(b => {
  //           const source: Observable<any>[] = [];
  //           b.data.forEach(doc => source.push(
  //             this.isqService.getByGuid(doc.bl_fi_generic_doc_hdr.guid.toString(), apiVisa).pipe(
  //               map((b_a) => {
  ...
  //                     link.guid_doc_1_line === doc.bl_fi_generic_doc_line[0].guid && link.txn_type === 'ISQO_ISI') {
