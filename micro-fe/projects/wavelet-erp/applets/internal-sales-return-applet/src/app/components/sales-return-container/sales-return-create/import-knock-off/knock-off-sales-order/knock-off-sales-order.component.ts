...
import {
  ...
  GenericDocServerDocTypeService,
  Pagination,
  SubQueryService,
  BaseGenDocTypeService,
  InternalSalesOrderService,
  BranchService,
  CustomerHeaderService,
  InternalPurchaseRequisitionService,
...
@Component({
  selector: "app-knock-off-sales-order",
  templateUrl: "./knock-off-sales-order.component.html",
  styleUrls: ["./knock-off-sales-order.component.css"],
})
export class KnockOffSalesOrderComponent extends ViewColumnComponent {
  ...
  retrieveData() {
    ...
    this.subs.sink = this.internalSalesOrderService
  ...
  onKO() {
    ...
    this.viewModelStore.dispatch(
      Column2ViewModelActions.setKOForImport_SODocNo({
        koForSODocNo: hdrServerDoc,
      })
    );
  ...
  onSelectionChanged(event: SelectionChangedEvent) {
    ...
    this.subs.sink = this.viewModelStore
      .select(Column2ViewSelectors.selectKOForImport_SODocNo)
      .subscribe((result) => {
        if (result === serverDoc) {
  ...
  onSearch(e: SearchQueryModel) {
    if (!e.isEmpty) {
      const sql = {
        subquery:
          e.queryString +
          `AND queue.server_doc_type_1 = 'INTERNAL_SALES_ORDER' AND queue.server_doc_type_2 = 'INTERNAL_SALES_RETURN_REQUEST'`,
        table: e.table,
      };
