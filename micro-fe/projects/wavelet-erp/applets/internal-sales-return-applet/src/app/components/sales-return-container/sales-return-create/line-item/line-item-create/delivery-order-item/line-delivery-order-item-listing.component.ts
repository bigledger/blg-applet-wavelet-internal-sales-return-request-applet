...
import { ApiResponseModel, InternalOutboundDeliveryOrderService, Pagination, SubQueryService } from 'blg-akaun-ts-lib';
...
@Component({
  selector: 'app-line-delivery-order-item-listing',
  templateUrl: './line-delivery-order-item-listing.component.html',
  styleUrls: ['./line-delivery-order-item-listing.component.scss']
})
export class LineDeliveryOrderItemListingComponent implements OnInit, OnDestroy {
  ...
  columnsDefs = [
    { headerName: 'Delivery Order No.', field: 'doc_number', cellStyle: () => ({ 'text-align': 'left' }), floatingFilter: true },
  ...
  //       this.subs.sink = this.doService.getLinesByCriteria(this.pagination, apiVisa).pipe(
  //         mergeMap(b => {
  //           const source: Observable<any>[] = [];
  //           b.data.forEach(doc => source.push(
  //             this.doService.getByGuid(doc.bl_fi_generic_doc_hdr.guid.toString(), apiVisa).pipe(
  //               map((b_a) => {
  ...
  //                     link.guid_doc_1_line === doc.bl_fi_generic_doc_line[0].guid && link.txn_type === 'IODO_ISGRN') {
