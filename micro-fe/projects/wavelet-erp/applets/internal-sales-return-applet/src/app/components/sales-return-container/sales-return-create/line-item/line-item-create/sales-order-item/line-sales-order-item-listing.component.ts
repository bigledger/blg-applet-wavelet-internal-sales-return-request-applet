...
import { ApiResponseModel, InternalSalesOrderService, Pagination, SubQueryService } from 'blg-akaun-ts-lib';
...
export class LineSalesOrderItemListingComponent implements OnInit, OnDestroy {
  ...
  columnsDefs = [
    { headerName: 'Sales Order No.', field: 'doc_number', cellStyle: () => ({ 'text-align': 'left' }), checkboxSelection: true, floatingFilter: true },
                ...
                map((b_a) => {
                  ...
                  b_a.data.bl_fi_generic_doc_link.forEach(link => {
                    if (link.guid_doc_1_hdr === doc.bl_fi_generic_doc_hdr.guid &&
                      link.guid_doc_1_line === doc.bl_fi_generic_doc_line[0].guid && link.txn_type === 'ISO_IPO') {
                  ...
                  const data = {
                    ...
                    unit_price_std: doc.bl_fi_generic_doc_line[0].unit_price_std,
                    unit_price_txn: doc.bl_fi_generic_doc_line[0].unit_price_txn,
                    uom: doc.bl_fi_generic_doc_line[0].uom,
                    status: null,
                    tax_gst_code: doc.bl_fi_generic_doc_line[0].tax_gst_code,
                    tax_wht_code: doc.bl_fi_generic_doc_line[0].tax_wht_code,
                    item_txn_type: doc.bl_fi_generic_doc_line[0].item_txn_type,
                    item_sub_type: doc.bl_fi_generic_doc_line[0].item_sub_type,
                  }
                  return data;
                })
              )
  ...
  //                 b_a.data.bl_fi_generic_doc_link.forEach(link => {
  //                   if (link.guid_doc_1_hdr === doc.bl_fi_generic_doc_hdr.guid &&
  //                     link.guid_doc_1_line === doc.bl_fi_generic_doc_line[0].guid && link.txn_type === 'ISO_ISGRN') {
  //                     let contra = Number(link.quantity_signum) * Number(link.quantity_contra);
  //                     aggContra += contra;
  //                   }
  //                 });
