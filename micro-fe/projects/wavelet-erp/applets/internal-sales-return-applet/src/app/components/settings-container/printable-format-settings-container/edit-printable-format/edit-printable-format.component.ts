...
export class EditPrintableFormatComponent extends ViewColumnComponent {
  ...
  ngOnInit() {
    ...
    this.subs.sink = this.printableData$.subscribe((resolved: any) => {
      ...
      this.extGuid = resolved.bl_prt_printable_format_exts[0].guid;
      this.fileName = resolved.file.name;
    });
  }
  ...
  onDownload() {
    this.printableFormatService.getFile(this.extGuid, AppConfig.apiVisa)
