...
@Component({
  selector: 'app-edit-line-item-bin-number-listing',
  templateUrl: './bin-number-listing.component.html',
  styleUrls: ['./bin-number-listing.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush,
  providers: [ComponentStore]
})
export class EditLineItemBinNumberListingComponent extends ViewColumnComponent {
  ...
  onGridReady(params) {
    ...
    this.invItem$.pipe(
      ...
      if (inv && sales_return.bl_fi_generic_doc_hdr.guid_store) {
  ...
  setGridData() {
    ...
    const datasource = {
      getRows: grid => {
        ...
        this.subs.sink = this.binService.getByCriteria(page, apiVisa).subscribe(
          { next: (resolve: any) => {
  ...
  onRowClicked(bin: BinModel) {
    this.store.dispatch(LineItemActions.selectBin({ bin: bin.bl_inv_bin_hdr }));
  }
