...
export class ItemDetailsMainComponent extends ViewColumnComponent {
  ...
  popNewItem() {
    this.subs.sink = this.store.select(SalesReturnSelectors.selectLineItem).subscribe({ next: resolve => {
      if (resolve) {
  ...
  popLineItem() {
    this.subs.sink = this.store.select(SalesReturnSelectors.selectLineItem).subscribe({ next: (resolve: any) => {
      if (resolve) {
        this.form.patchValue({
          ...
          unitPriceUom: resolve.item_property_json.unitPriceUom ? parseFloat(resolve.item_property_json?.unitPriceUom).toFixed(2) : 0,
          unitDiscountUom: resolve.item_property_json.unitDiscountUom ? parseFloat(resolve.item_property_json.unitDiscountUom).toFixed(2) : 0,
          qty: resolve.quantity_base,
          unitPrice: resolve.item_property_json.unitPrice ? parseFloat(resolve.item_property_json.unitPrice).toFixed(2) : 0,
          unitDiscount: resolve.item_property_json.unitDiscount ? parseFloat(resolve.item_property_json.unitDiscount).toFixed(2) : 0,
          discountAmt: resolve.amount_discount ? parseFloat(resolve.amount_discount).toFixed(2) : 0,
          stdAmt: resolve.amount_std ? parseFloat(resolve.amount_std).toFixed(2) : 0,
          netAmt1: resolve.amount_net ? parseFloat(resolve.amount_net).toFixed(2) : 0,
          taxCode: resolve.item_property_json?.taxCode,
          taxPercent: resolve.tax_gst_rate ? parseFloat(resolve.tax_gst_rate).toFixed(2) : 0,
          taxAmt: resolve.amount_tax_gst ? parseFloat(resolve.amount_tax_gst).toFixed(2) : 0,
          netAmt2: resolve.item_property_json.netAmt2 ? parseFloat(resolve.item_property_json.netAmt2).toFixed(2) : 0,
          whtCode: resolve.item_property_json?.whtCode,
          whtPercent: resolve.tax_wht_rate ? parseFloat(resolve.tax_wht_rate).toFixed(2) : 0,
          whtAmt: resolve.amount_tax_wht ? parseFloat(resolve.amount_tax_wht).toFixed(2) : 0,
          txnAmt: resolve.amount_txn ? parseFloat(resolve.amount_txn).toFixed(2) : 0,
          remarks: resolve.item_remarks,
          itemTxnType: resolve?.item_txn_type,
        });
        this.itemType = resolve.item_property_json?.itemType;
      }
    }});
    this.item$ = this.fiService.getByGuid(this.form.value.itemGuid, this.apiVisa).pipe(
      map(a => a.data)
