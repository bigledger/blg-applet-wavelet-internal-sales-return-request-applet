...
@Component({
  selector: 'app-header-doc-link-copy-to',
  templateUrl: './copy-to.component.html',
  styleUrls: ['./copy-to.component.scss']
})
export class HeaderDocLinkCopyToComponent implements OnInit, OnDestroy {
  ...
  setGridData() {
    const datasource = {
      getRows: grid => {
        this.subs.sink = combineLatest([this.pns$, this.links$]).pipe(
          mergeMap(([a, b]) => {
            const source: Observable<any>[] = [];
            let match = false;
            a.forEach(line => {
              b.forEach(link => {
                if (link.guid_doc_1_line === line.guid) {
                  match = true;
                  source.push(
