...
export class CompanyContainerComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private pagesService: CompanyPagesService) {
  }

  ngOnInit() {
    this.subs.sink = this.viewColFacade.companyViewCache$.subscribe(a =>
  ...
  ngOnDestroy() {
    this.viewColFacade.saveCompanyState();
